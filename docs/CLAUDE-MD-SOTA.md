...
Authoritative reference for CLAUDE.md generation. **Seed version**: web-sourced best practices only. Run `/refresh-guidelines` to produce `CLAUDE-MD-SOTA.enriched.md` (seed + `/insights` data, gitignored).

## Purpose & Scope

This **seed file** defines the standards for CLAUDE.md files. It contains web-sourced best practices only:
...
Use the enriched version when writing CLAUDE.md files; fall back to this seed when the enriched version is unavailable.

**Document structure:**

| Part | Coverage |
|------|----------|
| Part 1: Structural Standards | CLAUDE.md skeleton — sections, order, size budget |
| Part 2: Content Guidelines | What goes in each section and what to exclude |
| Part 3: Integration Guidelines | What goes in CLAUDE.md vs hooks vs skills vs rules |
| Part 4: Behavioral Patterns | Patterns selected and adapted based on project characteristics |
| Part 5: Maintenance Guidelines | Maintenance section guidance |
...
CLAUDE.md operates within a multi-level memory hierarchy:

- **`CLAUDE.md`** (project root) — Main project instructions, read every session.
- **`CLAUDE.local.md`** (project root, gitignored) — Personal preferences and local overrides. Mentioned in maintenance section.
...
7. **Installed Entities** — Agents, skills, hooks installed (optional; add when using a catalog tool).
8. **Known Gaps** — Technologies not covered by installed entities (optional).
...
Use `claude /init` for a baseline, but always curate every line. Start simple, expand based on friction points.
